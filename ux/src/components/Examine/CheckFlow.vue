  ...
  <div
    ...
    <div class="flow-body">
      <flexbox
        ...
        justify="flex-start">
        <i
          :class="item.examineStatus == 11 ? 'wk wk-icon-success' : getStatusIcon(item.examineStatus)"
          :style="{
            color:item.examineStatus == 11 ? '#00875A' : getStatusColor(item.examineStatus)
          }"
        ...
        <div>
          ...
          <flexbox class="cf-flow-item-info">
            ...
            <!-- 审核中 3 当做 待审核 0 处理 -->
            <div v-if="item.examineStatus != 11 && item.examineStatus != 12"><span>{{ item.examineStatus != 16 ? getStatusName(item.examineStatus) + '此申请' : '' }}</span></div>
