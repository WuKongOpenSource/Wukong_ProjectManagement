...
export function wkPreviewFileUrl(path, name) {
  return `${WKConfig.getLocationOrigin()}/file/preview?url=${encodeURIComponent(`${path}${path.includes('?fullfilename=') ? '' : `?fullfilename=${name || ''}`}&c=${axios.defaults.headers.common['Admin-Token']}`)}`
}
