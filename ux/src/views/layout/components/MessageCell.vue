  ...
  computed: {
    ...
    typeObj() {
      ...
      if (this.data.label && (this.data.label <= 5 || [8, 9].includes(this.data.label))) {
        if (this.data.label <= 5) {
          ...
          if ([53, 54, 55, 150].includes(this.data.type)) {
            key = 'jxc_purchase'
          } else if ([56, 57, 58, 151].includes(this.data.type)) {
            key = 'jxc_retreat'
          } else if ([59, 60, 61, 152].includes(this.data.type)) {
            key = 'jxc_sale'
          } else if ([62, 63, 64, 153].includes(this.data.type)) {
            key = 'jxc_saleReturn'
          } else if ([65, 66, 67, 154].includes(this.data.type)) {
            key = 'jxc_payment'
          } else if ([68, 69, 70, 155].includes(this.data.type)) {
            key = 'jxc_collection'
          } else if ([71, 72, 73, 156].includes(this.data.type)) {
            key = 'jxc_inventory'
          } else if ([74, 75, 76, 157].includes(this.data.type)) {
            key = 'jxc_allocation'
          }
        ...
        if ([1, 2, 3].includes(this.data.type)) {
          ...
          key = 'announcement'
        } else if ([10, 11, 24, 26, 30, 33, 146].includes(this.data.type)) {
          key = 'contract'
        } else if ([12, 13, 27, 147].includes(this.data.type)) {
          key = 'receivables'
          ...
          key = 'business'
        } else if ([35, 36, 37, 148].includes(this.data.type)) {
          key = 'invoice'
        } else if ([41].includes(this.data.type)) {
          key = 'knowledge'
        } else if ([43, 44].includes(this.data.type)) {
          key = 'visit'
        }
      }
      return typesObj[key] || {
    ...
    leftContent() {
      return {
        ...
        43: `${this.$getUserName(this.data)} 导入回访数据${this.data.title}条，${this.getImportContent(this.data)}`,
        44: `${this.$getUserName(this.data)} 取消导入回访数据，已导入${this.data.title}条，${this.getImportContent(this.data)}`,
        ...
        136: `${this.$getUserName(this.data)}评论了`,
    ...
    isImportType() {
      ...
      [50, 81, 82, 43, 44].includes(this.data.type)
    },
